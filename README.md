...
Проект настроен для автоматического запуска тестов с помощью **GitHub Actions**. Pipeline запускается при каждом push или pull request в ветку `master` и выполняет следующие шаги:

- Клонирует репозиторий.
- Устанавливает Java 11 и Maven.
- Кэширует зависимости Maven для ускорения сборки.
- Запускает тесты с помощью `mvn clean test`.
- Устанавливает Python 3.8 и зависимости из `requirements.txt`.
- Проверяет доступность Flask API.

### Конфигурация pipeline
Файл конфигурации находится в `.github/workflows/ci-pipeline.yml`. Основные шаги:

1. **Checkout репозитория**: Клонирование кода.
2. **Установка Java 11**: Настройка JDK.
3. **Кэширование Maven-зависимостей**: Ускорение сборки.
4. **Запуск тестов**: Выполнение `mvn clean test`.
5. **Установка Python**: Настройка Python 3.8 и зависимостей.
6. **Проверка Flask API**: Проверка доступности скрипта API.

### Просмотр результатов
- После push/pull request перейдите в раздел **Actions** вашего репозитория на GitHub.
- Вы увидите статус выполнения pipeline и логи для каждого шага.
